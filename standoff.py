...
def kill(aggressor, target):
    aggressor.pointing_gun_at[target.name] = False
    target.dead = True


def kill_effect(aggressor, target):
    return Effect(lambda: kill(aggressor, target),
                  lambda: "{0} is dead, killed by {1}".format(target.name, aggressor.name))

def threaten(aggressor, target):
    aggressor.pointing_gun_at[target.name] = True
...
def kill_event(aggressor, target):
    return Event(preconditions=[alive_precondition(aggressor),
                                ...
                                threatened_precondition(aggressor, cowboys - {aggressor}).negation()],
                 effects=[kill_effect(aggressor, target)],
                 description=lambda: "{0} kills {1}".format(aggressor.name, target.name))


def threaten_event(aggressor, target):
    return Event(preconditions=[alive_precondition(aggressor),
                                alive_precondition(target),
...
events = []
for first, second in itertools.combinations(cowboys, 2):
    events.append(kill_event(first, second))
    events.append(kill_event(second, first))
    events.append(threaten_event(first, second))
    events.append(threaten_event(second, first))

standoff = Env(events, 100)
standoff.run()
